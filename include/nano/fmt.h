...
int     vsnprint(char *buf, int len, char *fmt, va_list args);
        ...
        // Ethos-specific additions

        // defined in environment
void    print(const char buff[]);
void    printLog(const char buff[]);

int     printf(const char *format, ...);
int     printfLog(const char *format, ...);

void    xprint(const char *format, ...);
void    xprintLog(const char *format, ...);

void    vxprint(const char *format, va_list args);
void    vxprintLog(const char *format, va_list args);

        // print to string
String *xprintString(const char *format, ...);
String *vxprintString(const char *format, va_list args);

int     vsscanf(const char * buf, const char * fmt, va_list args);
int     sscanf(const char * buf, const char * fmt, ...);

char    toHex(int);
String *binaryToHexName(uchar *c, uint32 size);
String *uint64ToHexString(uint64 s);
#endif
